...
export default function Surveys() {
  ...
  const fetchSurveys = async () => {
    ...
    const activeUsingSurvey = snapshot.docs
        ...
        .some(a => a.surveyId === surveyId && a.participants?.some(p => !p.completed));

    if (activeUsingSurvey) {
        alert("Cannot delete this survey because it is used in an active assessment.");
        return;
    }
  ...
  useEffect(() => {
    ...
    const exists = surveys.some(s => s.name.toLowerCase() === trimmedName.toLowerCase());
    if (exists) {
    ...
    const newName = prompt("Enter a name for the new survey based on this template:");
    if (!newName) return;

    const trimmedName = newName.trim();
    if (surveys.some(s => s.name.toLowerCase() === trimmedName.toLowerCase())) {
        alert("A survey with this name already exists!");
        return;
    }
  ...
  // Open Edit Survey modal
    ...
    const activeUsingSurvey = snapshot.docs
        .map(doc => ({ id: doc.id, ...doc.data() }))
        .some(a => a.surveyId === survey.id && a.participants?.some(p => !p.completed));

    if (activeUsingSurvey) {
        alert("Cannot edit this survey because it is used in an active assessment.");
        return;
    }
  ...
  const handleRemoveQuestionFromSurvey = async (questionId) => {
    const updatedQuestions = editingSurvey.questions.filter(q => q.id !== questionId);
    const surveyRef = doc(db, "surveys", editingSurvey.id);
    await updateDoc(surveyRef, { questions: updatedQuestions });
    setEditingSurvey({ ...editingSurvey, questions: updatedQuestions });
    fetchSurveys();
        ...
        {surveys.length > 0 ? surveys.map(s => (
          ...
          >
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              ...
              <button onClick={() => handleUseTemplate(s)}>Use as Template</button>
              <button
        ...
        >
          ...
          >
            ...
            <button
              ...
              style={{ float: "right", backgroundColor: "#2fd334", marginBottom: "12px" }}
            >
              Close & Save
            </button>
            ...
            <h><em>**Questions appear in the order that you add them</em></h>
            {editingSurvey.questions.length > 0 ? editingSurvey.questions.map(q => (
              <div
                key={q.id}
                className="user-card"
            ...
            {filteredQuestions.length > 0 ? filteredQuestions.map(q => (
              ...
              >
                <span>{q.text} ({q.type})</span>
                <button onClick={() => handleAddQuestionToSurvey(q)}>Add</button>
              </div>
